...
在信号量模型中，包含了计数器和等待队列，以及对它们的操作方法，初始化Semaphore时，会初始化计数器数量，也就是最大并发的线程数量，类似互斥锁，进入临界区时，使用down()方法，计数器减1，离开临界区时，使用up方法，计数器+1，当计数器变为0时，其它的线程无法进入临界区，在等待队列中；
Semaphore相较于Lock锁，同样实现了互斥的功能，但Semaphore能允许多个线程同时进入临界区，有限流的作用（类似停车场功能）；


# 读写锁 ReadWriteLock
对于读操作多，写操作少的并发场景，读写锁满足的特性：
- 允许多个线程同时进行读操作
- 同一时间只允许一个线程写操作
- 如果某个线程在进行写操作，禁止线程读操作变量
...
# StampedLock
相较于ReadWriteLock，针对读多写少的场景，具有更高的效率；包括写锁，悲观读锁和乐观读；写锁，悲观读锁，与ReadWriteLock的读写锁类似，只允许一个线程写操作，允许多个线程悲观读操作，写锁和悲观读锁是互斥的；乐观读并不存在锁锁；
乐观读升级为悲观读锁；

## 乐观锁&悲观锁

# CountDownLatch&CyclicBarrier
CoutDownLatch能够实现一个线程等待另一组线程完成，比如线程A，B并发执行，在A,B都完成后，继续执行线程C，其中线程C等待A，B都完成的逻辑；
CyclicBarrier能够实现一组线程相互等待，并具有循环执行的功能，比如一组线程A，B，C并发执行，线程A，B分别完成一次执行逻辑后，线程C收到通知后继续执行，同时，A，B继续下一次执行，如此循环往复；
...
# 原子类
  是一种无锁的解决并发问题的方案，因为不需要加锁，释放锁，所以性能比互斥锁并发方案更高；其原理是CPU为了解决并发问题，提供了CAS指令（Compare And Swap），CAS指令本身具有原子性，利用比较并交换原则，