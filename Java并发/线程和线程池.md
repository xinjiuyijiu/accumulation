...
```
    private class ScheduledFutureTask<V>
        ...
        int heapIndex;
```
创建ScheduledThreadPoolExecutor线程池，实现了ScheduledExecutorService接口，能够延迟或者周期性的执行任务；workQueue是DelayedWorkQueue，它是一个基于堆数据结构的，根据任务延时时间，对任务进行排序；如果瑟吉欧Runnable任务会被封装成ScheduledFutureTask结构，它包含了任务延迟执行和周期执行的信息；
