...
```
corePoolSize为0，maximumPoolSize为Integer.MAX_VALUE，表示所有的工作线程都不是核心线程；工作线程数量不固定，跟随任务规模而增长；没有核心线程，则他们都有可能被回收；keepAliveTime为60s，则表示60s内没有任务执行的线程会被回收，或者说缓存时间为60s；workQueue是SynchronousQueue，
